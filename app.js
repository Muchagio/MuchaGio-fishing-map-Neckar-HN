...
const APP_VERSION=data?.meta?.version||'1.0.0';
...
function initMap(){
  ...
  async function loadMapData(){
    ...
    for(const point of data.structurePoints){
      ...
      marker.on('click',()=>showDetail(point,position,`${Math.round(waterHit.distance)} m zur geladenen Gewässergeometrie`));
      L.circle(position,{radius:point.reviewRadiusM||50,color:'#a66cf2',weight:2,dashArray:'7 7',opacity:.95,fillColor:'#a66cf2',fillOpacity:.12,interactive:true})
        .addTo(reviewAreaLayer)
        .bindPopup(popupHtml(`Prüfbereich: ${point.title}`,'Prüfhinweis rund um das verifizierte Bauwerk. Die tatsächlichen Sperr- und Sicherheitsabstände ergeben sich ausschließlich aus der Originalkarte und der Beschilderung vor Ort.',point.sectionId,'warning',`Radius ${point.reviewRadiusM||50} m als Orientierung, nicht als amtliche Sperrgrenze.`));
    ...
    sections.forEach(section=>{
      ...
      L.marker(nearest.latlng,{icon:sectionIcon(section.number),zIndexOffset:500})
        ...
        .bindPopup(popupHtml(section.title,section.subtitle,section.id,'allowed','Abschnittsmarker automatisch auf die geladene Gewässergeometrie gesetzt.'));
      allBounds.extend(nearest.latlng);
    });

    setRuntimeStatus(`GIS aktiv: ${waterLoaded}/${data.osmWaterSources.length} Gewässerbereiche geladen. ${structuresShown} verifizierte Bauwerke samt Prüfbereich angezeigt; ${structuresHidden} nicht ausreichend sichere Bauwerkspunkte ausgeblendet. Abschnittsmarker liegen direkt auf der geladenen Gewässergeometrie.`,waterLoaded===data.osmWaterSources.length?'ok':'warning');
